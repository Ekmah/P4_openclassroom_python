...
def algo(tournament_id, new):
    ...
    for player_id in tournament.players_id:
        ...
        compiled_player = {"player_id": player_id, "elo": player.elo,
                           "player_score": player_score,
                           "player_score_id": player_score_id,
    ...
    for player_1 in group_1:
        for player_2 in group_2:
            if not player_1["player_score"].player_already_matched_id:
                ...
                player_1["player_score"].player_already_matched_id = [
                    player_2['player_id']]
                player_2["player_score"].player_already_matched_id = [
                    player_1['player_id']]
                ...
                break
            elif player_2["player_id"] not in \
                    player_1["player_score"].player_already_matched_id:
                match_id = Match(False).save()
                # print(match_id)
                StatutPlayer(match_id, player_1["player_id"]).save()
                ...
                player_1["player_score"].player_already_matched_id.append(
                    player_2['player_id'])
                player_2["player_score"].player_already_matched_id.append(
                    player_1['player_id'])
                player_1["player_score"].update(player_1['player_score_id'])
                player_2["player_score"].update(player_2['player_score_id'])
